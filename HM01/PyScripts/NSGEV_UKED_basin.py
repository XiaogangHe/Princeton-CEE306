...
def fitGEV(ID, stationary=True):   
    ...
    if stationary == True:
        r("fit_obj <- gev.fit(-ts)")    # For low flow, take the negative value of the input 
        ...
        #r("fit_obj <- gev.fit(-ts, ydat=matrix(1:length(ts), ncol=1), mul=1, sigl=1, siglink=exp)")    # log(sigma(t))=sigma0+sigma1*t
        pars = r("fit_obj$mle")         # location, scale, shape
...
def fitGEV_LRT(ID, covariate):   
    ...
    r("fit2 <- fevd(-ts, location.fun=~covariate_valid, scale.fun=~covariate_valid, use.phi=TRUE, type=c('GEV'))") 
...
def get_basin_latlonarea():

    """
    Universal Kriging with External Drift for each basin

    Args:
        :siteLat (array): latitude for the gauges 
        :siteLon (array): longitude for the gauges 
        :siteArea (array): drainage area for the gauges 
        :GEVpar (array): GEV parameters 
    """

    basin_latlonarea = np.loadtxt('../USGS_GageII/basin_latlonarea_northeast.txt')

    return basin_latlonarea


def UKED_basin(siteLat, siteLon, siteArea, GEVpar):
    ...
    HCDN_lon = siteLon
    HCDN_area = siteArea
    basin_latlonarea = get_basin_latlonarea()
    basin_lat = basin_latlonarea[:, 0]
    basin_lon = basin_latlonarea[:, 1]
    basin_area = basin_latlonarea[:, 2]

    ### Convert numpy to R format
    r.assign('HCDN_lat', FloatVector(HCDN_lat))
    r.assign('HCDN_lon', FloatVector(HCDN_lon))
    r.assign('HCDN_area', FloatVector(HCDN_area))
    r.assign('GEVpar', FloatVector(GEVpar))
    r.assign('basin_lat', FloatVector(basin_lat))
    r.assign('basin_lon', FloatVector(basin_lon))
    r.assign('basin_area', FloatVector(basin_area))
    ...
    ### Create the grid for spatial predictions
    r("data.HCDN <- data.frame(lon=HCDN_lon, lat=HCDN_lat, GEVpar=GEVpar, area=HCDN_area)")
    r("data.basin <- data.frame(lon=basin_lon, lat=basin_lat, area=basin_area)")
    r("coordinates(data.HCDN) <- ~lon+lat")
    r("coordinates(data.basin) <- ~lon+lat")

    ##### Automaticly fit the variogram using 'automap' package and do the universal kriging
    r("data.kriged <- autoKrige(GEVpar~lon+lat+area, data.HCDN, data.basin)")
    kgpred = np.array(r("data.kriged$krige_output$var1.pred"))
...
def plot_yp_trend(yp_trend):
    ...
    plt.figure()
    M = Basemap(resolution='c', llcrnrlat=dims['minlat'], urcrnrlat=dims['maxlat'], llcrnrlon=dims['minlon'], urcrnrlon=dims['maxlon'])
    M.imshow(np.ma.masked_equal(yp_trend, -9.99000001e+08), cmap='RdYlBu_r', vmax=0.15, vmin=-0.15)
    #M.imshow(np.ma.masked_equal(yp_trend, -9.99000001e+08), cmap='Spectral_r')
    M.colorbar()
    plt.savefig('%s/UKED_ypp_year_v2.png' % (fig_dir), dpi=400)
...
NAO = np.loadtxt('%s/NAO.latest' % (clidir))[:,2].reshape(-1,12)
NAO = np.column_stack([np.arange(1950,2016), NAO])
...
NSUKED(gageID_long)
UKED_mu0_basin = UKED_basin(gageLat[gageID_long.index].values, gageLon[gageID_long.index].values, gageDrainage[gageID_long.index].values, gageInfo['mu0'][gageID_long.index])
