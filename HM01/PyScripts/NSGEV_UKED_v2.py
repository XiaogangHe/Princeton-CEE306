...
def fitGEV_climatic(ID, cliIndices):   
    ...
    r("fit_obj <- gev.fit(-ts, ydat=matrix(cliIndices_valid, ncol=1), mul=1)")
    pars = r("fit_obj$mle")    # location, scale, shape
...
def fitGEV_LRT(ID, covariate):   
    ...
    location_ts = r("location_ts <- -findpars(fit11)$location")    # For low flow 
    scale_ts = r("findpars(fit11)$scale")
    shape_ts = r("findpars(fit11)$shape")
    nllh = r("summary(fit11)$nllh")
    BIC = r("summary(fit11)$BIC")
    r("fit_mu <- lm(location_ts~covariate_valid)")
    mu_0 = r("fit_mu$coefficients[[1]]")
    mu_1 = r("fit_mu$coefficients[[2]]")

    return location_ts, scale_ts, shape_ts, pvalue_mu1, mu_0, mu_1, nllh, BIC, covariate_valid 
...
def NSUKED(gageID_select):
    ...
    for i in range(nSelect):
        cov_test = SOI[gageStime[gageID_select.index].values[i]-1876:-1, 1:4].mean(-1) 
        ...
        if pv_TFPW[i] <= 0.05:
            ...
            pv_mu1 = fitGEV_LRT('0%s' % (gageID_select.values[i]), cov_test)[3] 
            mu0, mu1, sigma, xi  = fitGEV_climatic('0%s' % (gageID_select.values[i]), cov_test) 
            gageInfo['mu0'][indSelect[i]] = np.array(mu0)
            gageInfo['mu1'][indSelect[i]] = np.array(mu1)
            gageInfo['sigma0'][indSelect[i]] = np.array(sigma)
            gageInfo['sigma1'][indSelect[i]] = 0
            gageInfo['xi'][indSelect[i]] = np.array(xi)
