...
pvalue_sigma = np.array([fitGEV_LRT('0%s' % (gageID_long.values[i]), np.arange(gageStime[gageID_long.index].values[i],2015))[4] for i in range(gageID_long.shape[0])])
...
dem_NEUS = np.ma.array(dem, mask=np.resize(mask_NEUS,dem.shape))

plt.figure()

mcolors = plt.cm.gray_r(linspace(0.2, 0.8, 100))
cmap = ListedColormap(mcolors)
M = Basemap(resolution='c', llcrnrlat=dims['minlat'], urcrnrlat=dims['maxlat'], llcrnrlon=dims['minlon'], urcrnrlon=dims['maxlon'])
#M.imshow(dem_NEUS, cmap='terrain')
M.imshow(dem_NEUS, cmap=cmap)
M.scatter(lon[ind], lat[ind], 120, trend[ind], vmin=0, vmax=0.6, cmap='YlOrRd')
cb = M.colorbar(ticks=arange(0, 0.61, 0.1))
cb.solids.set_edgecolor("face")
M.scatter(lon[ind2], lat[ind2], color='k', s=80, marker='^')
plt.savefig('/home/wind/hexg/Dropbox/Research_Princeton/GeneralExam/Results/Q7_trend.pdf', format='PDF')
plt.show()

### Plot the p-value for mu1 and sigma1
ind_sig = (pvalue_mu.squeeze()<=0.05) & (ps<=0.05)
#ind_sig = (pvalue_sigma.squeeze()<=0.05) & (ps<=0.05)
ind_nosig = (pvalue_mu.squeeze()>0.05) & (ps<=0.05)
#ind_nosig = (pvalue_sigma.squeeze()>0.05) & (ps<=0.05)
...
M = Basemap(resolution='c', llcrnrlat=dims['minlat'], urcrnrlat=dims['maxlat'], llcrnrlon=dims['minlon'], urcrnrlon=dims['maxlon'])
M.imshow(dem_NEUS, cmap=cmap)
M.scatter(lon[ind2], lat[ind2], color='k', s=80, marker='^', label='Stationary')           # stationary
M.scatter(lon[ind_sig], lat[ind_sig], 100, color='#ef4026', label='Significant')            # Non-stationary and significant different from 0
M.scatter(lon[ind_nosig], lat[ind_nosig], 100, color='#06c2ac', label='Not-significant')        # Non-stationary but not significant
plt.legend(loc=2)
plt.savefig('%s/LST_pvalue_mu1_time_Q7.pdf' % (fig_dir))
#plt.savefig('%s/LST_pvalue_sigma1_time_Q7.pdf' % (fig_dir))
plt.show()
