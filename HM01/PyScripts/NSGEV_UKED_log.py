...
def fitGEV_LRT(ID, covariate):   
    ...
    r("fit12 <- fevd(-ts, scale.fun=~covariate_valid, type=c('GEV'), use.phi=TRUE)")
    r("fit2 <- fevd(-ts, location.fun=~covariate_valid, scale.fun=~covariate_valid, use.phi=TRUE, type=c('GEV'))")    # use.phi=TRUE
...
def cal_ReturnLevelTrend(sigma0, mu1, sigma1, xi, p=0.01):
    yp_trend = mu1-sigma1/xi*np.exp(sigma0+sigma1*1951)*(1-np.power(-np.log(1-p),-xi))
    return yp_trend
